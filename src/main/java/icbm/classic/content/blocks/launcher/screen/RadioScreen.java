        ...
        if (canReceive(sender, packet)) {
            ...
            if (packet instanceof ITriggerActionMessage) {
                if (host.fireAllLaunchers(false)) { // TODO collect all screens and provide a single feedback message
                    ...
                    final double distance = Math.sqrt(target.squareDistanceTo(host.getPos().getX() + 0.5, host.getPos().getY() + 0.5,
                        host.getPos().getZ() + 0.5)); // TODO base from launcher
                    sender.onMessageCallback(this,
                        new TextMessage(getChannel(), RadioTranslations.RADIO_LAUNCH_SUCCESS, target.x, target.y, target.z, distance));
