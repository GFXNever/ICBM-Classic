    ...
    public List<TileEntity> getTilesInArea(World world) {
        ...
        for (Chunk chunk : getChunks(world)) {
            for (Object object : chunk.getTileEntityMap().values()) {
                if (object instanceof TileEntity && ((TileEntity) object).isInvalid() && ((TileEntity) object).getWorld() != null && isWithin(
                    ((TileEntity) object).getPos())) {
    ...
    protected void recalc() {
        if (canEdit) {
            if (pointOne != null && pointTwo != null) {
                lowerPoint =
                    new Pos(Math.min(pointOne.x(), pointTwo.x()), Math.min(pointOne.y(), pointTwo.y()), Math.min(pointOne.z(), pointTwo.z()));
                higherPoint =
                    new Pos(Math.max(pointOne.x(), pointTwo.x()), Math.max(pointOne.y(), pointTwo.y()), Math.max(pointOne.z(), pointTwo.z()));
