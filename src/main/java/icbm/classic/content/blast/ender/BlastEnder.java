...
public class BlastEnder extends Blast implements IBlastTickable //TODO handle save/load
{
    ...
    public boolean doExplode(int callCount) //TODO break into smaller methods
    {
                ...
                for (int z = -r; z < r; z++) {
                    for (int y = -r; y < r; y++) {
                        ...
                        if (distance < r && distance > r - 1) {
                            if (!targetPosition.getBlock(world())
                                .isAir(targetPosition.getBlockState(world()), world(), targetPosition.toBlockPos())) {
                                continue;
                            }
                            ...
                            if (this.world().rand.nextFloat() < Math.max(0.001 * r, 0.01)) {
                                ...
                                world.spawnParticle(EnumParticleTypes.PORTAL, targetPosition.x(), targetPosition.y(), targetPosition.z(), velX, velY,
                                    velZ);
                            }
                        }
                    }
        ...
        AxisAlignedBB bounds =
            new AxisAlignedBB(location.x() - radius, location.y() - radius, location.z() - radius, location.x() + radius, location.y() + radius,
                location.z() + radius);
            ...
            if (entity != this.controller) {
                ...
                if (new Pos(entity.posX, entity.posY, entity.posZ).distance(location) < 4) {
                    ...
                    try {
                        ...
                        this.world()
                            .playSound(null, entity.posX, entity.posY, entity.posZ, SoundEvents.ENTITY_ENDERMEN_TELEPORT, SoundCategory.BLOCKS, 1.0F,
                                1.0F);

                        if (entity instanceof EntityPlayerMP) {
                            ((EntityPlayerMP) entity).connection.setPlayerLocation(this.teleportTarget.x, this.teleportTarget.y,
                                this.teleportTarget.z, entity.rotationYaw, entity.rotationPitch);
                        } else {
        ...
        this.world()
            .playSound(null, this.location.x(), this.location.y(), this.location.z(), SoundEvents.BLOCK_PORTAL_AMBIENT, SoundCategory.BLOCKS, 2F,
