    ...
    protected final boolean isDataReadOnly(EventTrackerType type, Object[] data) {
        ...
        for (Object object : data) {
            if (object != null && !eventTrackerData.isValidType(object.getClass())) {
                good = false;
                ICBMClassic.logger()
                    .warn(String.format("EventTracker(%s): Class['%s'] is not supported for use with events.", type.getName(), object.getClass()));
            }
        }
        if (!good) {
            final String msg = String.format(
                "EventTracker(%s): Ignoring event due to containing editable fields. Supplying said fields could result in listeners incorrectly editing the game state",
                type.getName());
            ICBMClassic.logger().error(msg, new RuntimeException(
                "Dev Bug, likely caused by domain '" + type.getName().getResourceDomain() + "' but may be another mod using the domain incorrectly"));
        }
        return good;
    }
