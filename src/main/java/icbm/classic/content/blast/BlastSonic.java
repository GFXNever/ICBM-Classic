...
public class BlastSonic extends Blast implements IBlastTickable {
    ...
    public void firstTick() {
        ...
        ICBMSounds.SONICWAVE.play(world, location.x(), location.y(), location.z(), 4.0F,
                ...
                if (!getThreadResults().isEmpty()) {
                    ...
                    if (ConfigDebug.DEBUG_THREADS) {
                        String msg = String.format(
                            "BlastSonic#doPostExplode() -> Thread failed to find blocks to edit. Either thread failed or no valid blocks were found in range." +
                                "\nWorld = %s " +
                                "\nThread = %s" +
                                "\nSize = %s" +
                                "\nPos = %s",
