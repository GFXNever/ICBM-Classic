...
public class BlastEMP extends Blast {
    ...
    public boolean doExplode(int callCount) {
        if (!world().isRemote) {
                        ...
                        for (int z = (int) -this.getBlastRadius(); z < (int) this.getBlastRadius(); z++) {
                            ...
                            if (world.isBlockLoaded(blockPos)) {
                                ...
                                if (Math.round(location.x() + y) == location.yi()) {
                                    world().spawnParticle(EnumParticleTypes.SMOKE_LARGE, blockPos.getX() + 0.5, blockPos.getY() + 0.5,
                                        blockPos.getZ() + 0.5, 0, 0, 0);
                                }
                                        ...
                                        if (tileEntity != null) {
                                            ...
                                            if (tileEntity.hasCapability(CapabilityEMP.EMP, null)) {
                                                ...
                                                if (receiver != null) {
                                                    ...
                                                    doInventory =
                                                        receiver.shouldEmpSubObjects(world, tileEntity.getPos().getX(), tileEntity.getPos().getY(),
            ...
            ICBMSounds.EMP.play(world, location.x(), location.y(), location.z(), 4.0F,
                (1.0F + (world().rand.nextFloat() - world().rand.nextFloat()) * 0.2F) * 0.7F, true);
        }
        return true;
    }
    ...
    protected float empEntity(TileEntity entity, float powerEntity, IEMPReceiver receiver) {
        if (receiver != null) {
            powerEntity =
                receiver.applyEmpAction(world, entity.getPos().getX(), entity.getPos().getY(), entity.getPos().getZ(), this, powerEntity, true);
            //TODO spawn effects on entity if items were effected
            //TODO ICBMClassic.proxy.spawnShock(this.oldWorld(), this.position, new Pos(entity), 20);
        }
        return powerEntity;
