...
public class RadarMap {
    ...
    public List<RadarEntity> getRadarObjects(double x, double z, double distance) {
        return getRadarObjects(new Cube(x - distance, 0, z - distance, x + distance, ICBMClassic.RADAR_MAP_HEIGHT, z + distance).cropToWorld(), true);
    }
    ...
    public List<RadarEntity> getRadarObjects(Cube cube, boolean exact) {
        ...
        Pos minPosition = cube.min();
        Pos maxPosition = cube.max();

        int minimalChunkX = (minPosition.xi() >> 4) - 1;
        int maximalChunkX = (maxPosition.xi() >> 4) + 1;

        int minimalChunkZ = (minPosition.zi() >> 4) - 1;
        int maximalChunkZ = (maxPosition.zi() >> 4) + 1;

        for (int chunkX = minimalChunkX; chunkX <= maximalChunkX; chunkX++) {
            for (int chunkZ = minimalChunkZ; chunkZ <= maximalChunkZ; chunkZ++) {
                collectEntitiesInChunk(chunkX, chunkZ, (entity) -> {
                    if (!exact || cube.isWithin(entity)) {
                        list.add(entity);
                    }
                });
            }
