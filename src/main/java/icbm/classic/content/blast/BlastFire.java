...
public class BlastFire extends Blast {
    ...
    public boolean doExplode(int callCount) {
                            ...
                            for (float stepAmount = 0.3F; energy > 0.0F; energy -= stepAmount * 0.75F) {
                                ...
                                if (energy > 0.0F) {
                                    ...
                                    if (chance > distanceFromCenter * 0.55) {
                                        boolean canReplace =
                                            block.isReplaceable(world(), targetPosition) || block.isAir(blockState, world(), targetPosition);
        ...
        ICBMSounds.EXPLOSION_FIRE.play(world, location.x() + 0.5D, location.y() + 0.5D, location.z() + 0.5D, 4.0F,
