...
public class InventoryWithSlots extends ItemStackHandler implements ITick {
    ...
    public ItemStack insertItem(int slot, @Nonnull ItemStack stack, boolean simulate) {
        return isItemValid(slot, stack) && getSlot(slot).map(InventorySlot::getOnInsertStack).map(s -> s.apply(stack))
            .orElse(true) ? super.insertItem(slot, stack, simulate) : stack;
    }
